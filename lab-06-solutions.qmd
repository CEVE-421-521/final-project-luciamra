...
k_array = []
x = collect(0:0.01:9)
h = KernelDensity.default_bandwidth(float.(s))
for i in 1:length(x)
    #newdata = [rand(Normal(getindex(x,i), h)) for _=1:1000]
    c = target_pdf(getindex(x,i)) / p
    push!(k_array, c)
end
k = maximum(k_array)

#plot(q.x, k.*q.density, label="q(x)")
#plot!(target_pdf, 0, 9, label="p(x)")

## need to figur out how to get p
```
